...
OUTPUTFOLDER = os.path.abspath('td-generic/')
if not os.path.exists(OUTPUTFOLDER):
    os.makedirs(OUTPUTFOLDER)

CSVFILE = os.path.join(OUTPUTFOLDER, "miqcp-eval-generic.csv")

random.seed(42)


TASKS_BASE_PERIOD = [p * PERIOD_FACTOR for p in PRIMES[:TIMERS]]
TASKS = [random.choice(TASKS_BASE_PERIOD)*random.randint(1, RAND_FACTOR) for _ in range(NUM_TASKS)]
