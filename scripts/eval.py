...
def run_eval(period_factor=1, suffix=""):
    ...
    print(f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")} running fullclean')
    subprocess.run(['idf.py', 'fullclean'], cwd=PROJECT_PATH)

    print(f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")} running build+flash')
    subprocess.run(['idf.py', 'build', 'flash', '-p', device], cwd=PROJECT_PATH)
    print(f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")} finished build+flash')
    print(f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")} running test')
    ...
    insertion_stats = (statistics.mean(insertion_times), statistics.median(insertion_times), max(insertion_times), min(insertion_times), max(insertion_times)-min(insertion_times))
    retrieval_times = [int(t.split(',')[1]) for t in results_ret]
    retrieval_stats = (statistics.mean(retrieval_times), statistics.median(retrieval_times), max(retrieval_times), min(retrieval_times), max(retrieval_times)-min(retrieval_times))
...
run_eval(period_factor=-1, suffix="")
